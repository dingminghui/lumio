...
export async function handleItemChatPost(request: Request, { params }: RouteContext) {
  ...
  const userContent = latestUserMessage ? getMessageModelText(latestUserMessage) : "";
  ...
  try {
    ...
    return Response.json(
      {
        error: getErrorMessage(error, "模型环境变量配置无效"),
      },
      { status: 500 },
    );
  }
    ...
    async execute({ writer }) {
      ...
      if (finalResult.write === "commit" && finalResult.output) {
        try {
          ...
          const message = detail ? `节点状态写入失败：${detail}` : "节点状态写入失败";
