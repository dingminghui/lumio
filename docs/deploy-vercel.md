...
部署后站点默认 **对公网开放**（项目当前无登录鉴权），任何人可访问页面；AI 对话需在环境变量中配置 DeepSeek 模型凭证后才会可用。
...
节点对话若返回 `缺少环境变量 DEEPSEEK_*`，说明对应变量未在 Vercel 环境变量中配置。
...
当前版本 **没有用户登录**，所有访客共享同一数据库：

- 任何人可创建/修改画布项目数据。
- DeepSeek API Key 通过服务端环境变量注入，不会暴露给前端。

若面向公网长期开放，后续建议增加鉴权、按用户隔离数据。短期演示可接受上述风险。
